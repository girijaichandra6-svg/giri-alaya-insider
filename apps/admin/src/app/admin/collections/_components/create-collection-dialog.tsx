...
export function CreateCollectionDialog({ products: _products }: CreateCollectionDialogProps) {
  ...
  const [type, setType] = React.useState("EDITORIAL");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      addToast({
        title: "Error",
        description: err instanceof Error ? err.message : "Could not create collection",
        variant: "error",
      });
    } finally {
      setLoading(false);
      ...
      <div className="relative z-50 w-full max-w-2xl rounded-xl border border-white/10 bg-obsidian p-6 shadow-xl max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-heading text-lg font-medium">Create Collection</h3>
            <button
              type="button"
              onClick={handleClose}
