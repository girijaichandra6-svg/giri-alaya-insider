...
export function DeleteBrandButton({
  ...
  async function handleDelete() {
    ...
    try {
      ...
      addToast({
        title: "Error",
        description: err instanceof Error ? err.message : "Could not delete brand",
        variant: "error",
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-50 w-full max-w-md rounded-xl border border-white/10 bg-obsidian p-6 shadow-xl">
        <h3 className="font-heading text-lg font-medium mb-2">Delete Brand</h3>
        <p className="text-sm text-muted font-body mb-6">
          Are you sure you want to delete <strong className="text-softWhite">&ldquo;{brandName}&rdquo;</strong>?
