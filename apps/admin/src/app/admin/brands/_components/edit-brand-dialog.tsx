...
export function EditBrandDialog({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      addToast({ title: "Brand updated", variant: "success" });
      setOpen(false);
      router.refresh();
    } catch (err) {
      addToast({
        title: "Error",
        description: err instanceof Error ? err.message : "Could not update brand",
        variant: "error",
      });
    } finally {
      setLoading(false);
      ...
      <div className="relative z-50 w-full max-w-lg rounded-xl border border-white/10 bg-obsidian p-6 shadow-xl">
        <form onSubmit={handleSubmit}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-heading text-lg font-medium">Edit Brand</h3>
            <button
              type="button"
