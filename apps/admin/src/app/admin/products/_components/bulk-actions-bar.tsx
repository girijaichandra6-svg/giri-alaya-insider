...
export function BulkActionsBar({
  ...
  async function handleBulkAction(action: "delete" | "archive") {
    ...
    try {
      ...
      if (ids.length === 0) {
        addToast({ title: "Error", description: "No products selected", variant: "error" });
        return;
      }
      ...
      if (!res.ok) throw new Error(`Failed to ${action} products`);

      const label = action === "delete" ? "deleted" : "archived";
      addToast({
        title: `${ids.length} product${ids.length === 1 ? "" : "s"} ${label}`,
        variant: "success",
      });
      ...
      addToast({
        title: "Error",
        description: `Could not ${action} products`,
        variant: "error",
      });
    } finally {
      setLoading(null);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-50 w-full max-w-md rounded-xl border border-white/10 bg-obsidian p-6 shadow-xl">
            <h3 className="font-heading text-lg font-medium mb-2">
              {confirmAction === "delete" ? "Delete Products" : "Archive Products"}
            </h3>
            <p className="text-sm text-muted font-body mb-6">
              {confirmAction === "delete" ? (
                <>
                  ...
                  product{selectedCount === 1 ? "" : "s"}? This action cannot be undone.
                </>
              ) : (
                <>
                  Are you sure you want to archive <strong className="text-softWhite">{selectedCount}</strong>{" "}
                  product{selectedCount === 1 ? "" : "s"}? Archived products will be hidden from the storefront.
                </>
              )}
            </p>
