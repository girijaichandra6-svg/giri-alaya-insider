...
export function DeleteContentButton({
  ...
  async function handleDelete() {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/content?id=${postId}`, {
        method: "DELETE",
      });
      ...
      addToast({ title: "Error", description: "Could not delete post", variant: "error" });
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-50 w-full max-w-md rounded-xl border border-white/10 bg-obsidian p-6 shadow-xl">
        <h3 className="font-heading text-lg font-medium mb-2">Delete Post</h3>
        <p className="text-sm text-muted font-body mb-6">
          Are you sure you want to delete <strong className="text-softWhite">&ldquo;{postTitle}&rdquo;</strong>?
